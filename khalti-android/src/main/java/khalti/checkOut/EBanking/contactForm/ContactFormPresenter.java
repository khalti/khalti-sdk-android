...
class ContactFormPresenter implements ContactFormContract.Presenter {
    ...
    public void onCreate() {
        EBankingData eBankingData = view.receiveData();
        if (EmptyUtil.isNotNull(eBankingData)) {
            view.setBankData(eBankingData.getBankLogo(), eBankingData.getBankName(), eBankingData.getBankIcon());
            view.setButtonText("Pay Rs " + StringUtil.formatNumber(NumberUtil.convertToRupees(eBankingData.getConfig().getAmount())));
            compositeSubscription.add(view.setClickListener().subscribe(aVoid -> onFormSubmitted(view.isNetworkAvailable(), view.getContactNumber(), eBankingData.getBankIdx(),
                    eBankingData.getBankName(), eBankingData.getConfig())));
